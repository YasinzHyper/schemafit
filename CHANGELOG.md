...
- The fix for `openai/unsupported-composition` now inlines an `allOf` branch that is nothing but a local `$ref`, so the `{ "allOf": [{ "$ref": "#/$defs/User" }], "description": "..." }` that Pydantic emits for an annotated model field merges like any other branch. A `$ref` is not inlined when the definition it names refers back to itself, which cannot be inlined at all. The definition stays under `$defs` once its only use is inlined, unused but harmless.
- A definition that more than one subschema references is now inlined too, as a copy: the original stays where it is, for the references the fix does not touch, and the fix title says which definition was copied ("Merge the \"allOf\" branch into the object, copying #/$defs/user, which stays under \"$defs\" because the schema references it elsewhere too."). Two fields annotated with the same Pydantic model produce exactly this shape, and leaving it alone left an `allOf` that no fix could resolve and the API rejects. What the copy costs is size, and the property, nesting, and string-size rules measure it again over the fixed schema.
